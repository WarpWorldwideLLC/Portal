...
DROP PROCEDURE IF EXISTS getCartByReceipt;

DELIMITER $$
CREATE PROCEDURE getCartByReceipt(query JSON)
BEGIN
    ...
    SET ReceiptNumber := JSON_EXTRACT(query, '$.ReceiptNumber');
	...
	WHERE sc.RecordStatusID IN (10, 50)
      AND sc.ReceiptNumber = ReceiptNumber
